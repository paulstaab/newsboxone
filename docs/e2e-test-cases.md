...
| `TS-LOGIN-002`   | Valid credentials create a working browser session         | Backend auth is enabled with known credentials.               | Sign in through `/login` and then open `/timeline`.                        | The app stores a token-backed session under `newsboxone:session` and reaches the timeline.                              |
...
| `TS-FEED-MGMT-005` | Reader deletes a feed and its items                 | Existing feed has unread items.                                   | Delete the feed and then refresh feeds and items.                                                             | The feed disappears and its items are no longer returned; for newsletter feeds, later emails from the same sender do not recreate the feed. |
...
| `TS-FEED-MGMT-007` | Reader updates feed settings and quality overrides  | Existing regular feed is present.                                 | Open a feed settings dialog, update the title, folder, and tri-state quality overrides, then save the dialog. | The selected feed reflects the saved title, folder assignment, and manual quality overrides while staying visible in feed management.                       |
| `TS-FEED-MGMT-008` | Reader subscribes to a recommended RSS feed         | At least 5 regular feeds provide recommendation context, and deterministic mocked LLM and feed fixtures are available. | Open feed management, generate recommendations from the discovery modal, choose one recommendation, confirm folder selection, and return to the feed list. | The recommended feed is added through the normal create-feed path, appears in the selected folder, is removed from the modal results, and the modal remains usable. |
