...
It is built for homelab deployment: persistent local storage, required login protection for real deployments, optional AI-assisted summaries and feed recommendations, and a single image that serves both the frontend and the API.
...
- Use it locally with no cloud dependency, then optionally add OpenAI-compatible summaries and feed recommendations.
...
- Start without AI features first. Add `OPENAI_API_KEY` later if you want generated summaries, smarter newsletter parsing, or verified feed recommendations.
